...
public class UserServiceImpl extends BaseServiceImpl<User, Long, UserRepository> implements UserService, UserDetailsService {
	...
	@Override
	public User generateDefaultUser(String username) {
		User user=this.dao.getUserByUsername(StringUtils.lowerCase(username));
		if(user==null){
			user=new User();
		    ...
		    user.setEncryptPassword(EncryptUtil.encrypt(username));
		    user.setPassword(PasswordEncoder.encoder(username, username));
		    user.setRoles(roles);
		    this.dao.save(user);
		}
		return user;
