...
var Login = {
	...
	login : function(callback) {
		var username = $('#modal-login-form').find('input[name="username"]').val();
		var password = $('#modal-login-form').find('input[name="password"]').val();
		var rememberme = $('#modal-login-form').find('input[name="remember-me"]').is(':checked');
		$.ajax({
			url : '/security/web/ajax_j_spring_security_check',
			type : 'post',
			data : {
				username : username,
				password : password,
	...
	builder : function() {
		...
		sb.append('<input type="text" name="username" class="form-control" class="security" placeholder="邮箱或用户名" />');
		sb.append('</div>');
		sb.append('<div class="form-group">');
		sb.append('<input type="password" name="password" class="form-control" placeholder="输入密码" />');
		sb.append('</div>');
		sb.append('<div class="from-group">');
		sb.append('<input type="checkbox" name="remember-me" />');
	...
	isUserLogin:function(callback){
		$.ajax({
			url:'/security/web/userAction!isUserLogin.do',
			type:'post',
			dataType:'json',
			success:function(data,textStatus,xhr){
				if(!data.isUserLogin){
